...
export async function buildProducts(payload) {
  ...
  // payload.garments is an optional allow-list from the builder's checkboxes.
  const picked = Array.isArray(payload.garments) && payload.garments.length
    ? GARMENT_ORDER.filter((k) => payload.garments.includes(k))
    : GARMENT_ORDER;
  if (!picked.length) throw new Error('Pick at least one product to build.');
