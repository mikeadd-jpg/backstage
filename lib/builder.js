...
const PLACEMENT_FULL = { x: 0.5, y: 0.5, scale: 1.0, angle: 0 };
const PLACEMENT_LEFT_CHEST = { x: 0.82, y: 0.18, scale: 0.22, angle: 0 };
...
export function garmentList() {
  return GARMENT_ORDER.map((k) => ({ key: k, label: VARIANTS[k].label }));
}
...
export async function buildProducts(payload) {
  ...
  const frontId = await uploadImage(payload.frontB64, payload.frontName || 'front.png');
  let backId = null;
  if (payload.backB64) backId = await uploadImage(payload.backB64, payload.backName || 'back.png');
