...
// Aggregate view of support volume, for questions the dashboard has no screen for:
// what people are complaining about, and which brand is generating it.
export async function getSupportStats(days = 30) {
  const pool = getPool();
  const { rows: byType } = await pool.query(
    `SELECT brand, coalesce(issue_type, 'Unknown') AS issue_type,
            count(*)::int AS n,
            count(*) FILTER (WHERE status = 'open')::int AS still_open,
            count(*) FILTER (WHERE needs_action)::int AS needs_action
     FROM inquiries
     WHERE is_support = true AND received_at > now() - make_interval(days => $1)
     GROUP BY brand, issue_type
     ORDER BY n DESC`,
    [days]
  );
  ...
  return { days, totals: totals[0], byType };
}

export async function getInquiryById(id) {
  const { rows } = await getPool().query(
    `SELECT id, brand, from_email, to_email, subject, received_at, issue_type, summary, body,
            order_number, order_status, needs_action, draft_reply, confidence, status, resolved_at
